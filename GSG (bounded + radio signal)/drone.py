...
class drone(entities):
    ...
    def __init__(self,_canvas,_root, _drone_pos, _drone_signal, _adv_pos,_subarea_y,_subarea_x,_sub_y,_sub_x,_subarea_signal):
        ...
        self.cur_y_drone = random.randint(round(g_var.dimension*self.subarea_y[self.sub_y][0]),round(g_var.dimension*self.subarea_y[self.sub_y][1])-1)
        self.cur_x_drone = random.randint(round(g_var.dimension*self.subarea_x[self.sub_x][0]),round(g_var.dimension*self.subarea_x[self.sub_x][1])-1)
    ...
    def move_drone(self):
        ...
        if self.cur_x_drone<=g_var.dimension*self.subarea_x[self.sub_x][0]:
                lower_limit_x = 0
        elif self.cur_x_drone>=g_var.dimension*self.subarea_x[self.sub_x][1]-1:
            upper_limit_x = 0
        if self.cur_y_drone<=g_var.dimension*self.subarea_y[self.sub_y][0]:
            lower_limit_y = 0
        elif self.cur_y_drone>=g_var.dimension*self.subarea_y[self.sub_y][1]-1:
            upper_limit_y = 0
