...
class agent(entities):
    ...
    def __init__(self,_canvas,_root,_agent_pos,_cell_resources,_target_pos,_round_marking,_drone_signal,_subarea_y,_subarea_x,_sub_y,_sub_x, _adv_pos, _subarea_signal):
        ...
        self.cur_y_agent = random.randint(round(g_var.dimension*self.subarea_y[self.sub_y][0]),round(g_var.dimension*self.subarea_y[self.sub_y][1])-1)
        self.cur_x_agent = random.randint(round(g_var.dimension*self.subarea_x[self.sub_x][0]),round(g_var.dimension*self.subarea_x[self.sub_x][1])-1)
    ...
    def move_agent(self):
        ...
        if self.active_target==True:
            ...
            move_x, move_y = 0,0
            if x_offset>0 and self.cur_x_agent<g_var.dimension-1: move_x = +1
            elif x_offset<0 and self.cur_x_agent>0 : move_x=-1
            if y_offset>0 and self.cur_y_agent<g_var.dimension-1: move_y = +1
            ...
            if self.cur_x_agent<=g_var.dimension*self.subarea_x[self.sub_x][0]:
                lower_limit_x = 0
            elif self.cur_x_agent>=g_var.dimension*self.subarea_x[self.sub_x][1]-1:
                upper_limit_x = 0
            if self.cur_y_agent<=g_var.dimension*self.subarea_y[self.sub_y][0]:
                lower_limit_y = 0
            elif self.cur_y_agent>=g_var.dimension*self.subarea_y[self.sub_y][1]-1:
                upper_limit_y = 0
