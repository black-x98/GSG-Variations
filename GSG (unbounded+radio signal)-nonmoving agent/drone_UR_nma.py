...
class drone(entities):
    ...
    def __init__(self,_canvas,_root, _drone_pos, _drone_signal, _adv_pos):
        ...
        self.cur_x_drone = random.randint(0, g_var_UR_nma.dimension - 2)
        self.cur_y_drone = random.randint(0, g_var_UR_nma.dimension - 2)

        x_cor = self.cur_x_drone * g_var_UR_nma.block_size
        y_cor = self.cur_y_drone * g_var_UR_nma.block_size
        self.drone_pos[self.cur_y_drone][self.cur_x_drone] = 1
        self.canvas.create_oval(x_cor + 22, y_cor + 10, x_cor + 37, y_cor + 25, fill=self.drone_color, outline=g_var_UR_nma.bg_color)
    ...
    def move_drone(self):
        ...
        self.canvas.create_oval(x_cor + 22, y_cor + 10, x_cor + 37, y_cor + 25, fill=g_var_UR_nma.bg_color, outline=g_var_UR_nma.bg_color)
        ...
        if self.cur_y_drone<1:
            lower_limit_y = 0
        if self.cur_x_drone>g_var_UR_nma.dimension-2:
            upper_limit_x=0
        if self.cur_y_drone>g_var_UR_nma.dimension-2:
            upper_limit_y=0
        ...
        if self.drone_counter < g_var_UR_nma.movement_limit:
            self.root.after(g_var_UR_nma.turn_gap_time, self.move_drone)
