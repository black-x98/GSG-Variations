...
from agent_UR_nma import agent
from adversery_UR_nma import adv
from drone_UR_nma import drone
import g_var_UR_nma
...
class app(Frame):
    ...
    def refresh(self):
        ...
        self.label_arrest['text'] = "CAUGHT Poachers:\n" + str(g_var_UR_nma.arrested_poachers)
        self.label_fled['text'] = "FLED Poachers:\n" + str(g_var_UR_nma.fled_poachers)
        self.label_sack['text'] = "Resource Poached:\n" + str(g_var_UR_nma.resource_poached)
        self.label_recovered['text'] = "Resource Recovered:\n" + str(g_var_UR_nma.resource_recovered)
        self.label_travelled['text'] = "Distance Travelled:\n" + str(g_var_UR_nma.distance_travelled)

        for i in range(g_var_UR_nma.dimension): # cell resource label update
            for j in range(g_var_UR_nma.dimension):
                res_label = Label(self.root,text = self.cell_resources[j][i],bg="black",fg="white")
                res_label.place(x=i * g_var_UR_nma.block_size + 2, y=j * g_var_UR_nma.block_size + 2)

        self.refresh_counter += 1
        if self.refresh_counter <= g_var_UR_nma.movement_limit:
            #print("working in main refresh " + random.randint(1,50).__str__())
            self.root.after(g_var_UR_nma.turn_gap_time, self.refresh)
        elif self.refresh_counter > g_var_UR_nma.movement_limit:
    ...
    def __init__(self, _num_adv, _num_agents, _num_drones):
        ...
        g_var_UR_nma.num_of_adverseries = _num_adv
        g_var_UR_nma.num_of_agents = _num_agents
        g_var_UR_nma.num_of_drones = _num_drones

        # re-initialization of result variables

        g_var_UR_nma.arrested_poachers = 0
        g_var_UR_nma.fled_poachers = 0
        g_var_UR_nma.resource_poached = 0
        g_var_UR_nma.resource_recovered = 0
        g_var_UR_nma.distance_travelled = 0

        self.refresh_counter = 0
        g_var_UR_nma.movement_limit = 40
        ...
        self.agent_pos = [[0 for i in range(g_var_UR_nma.dimension)] for j in range(g_var_UR_nma.dimension)]
        #self.cell_resources = [[random.randint(10,50) for i in range(global_var.dimension)] for j in range(global_var.dimension)]
        self.adv_pos = [[0 for i in range(g_var_UR_nma.dimension)] for j in range(g_var_UR_nma.dimension)]
        self.drone_pos = [[0 for i in range(g_var_UR_nma.dimension)] for j in range(g_var_UR_nma.dimension)]
        self.drone_signal = [[0 for i in range(g_var_UR_nma.dimension)] for j in range(g_var_UR_nma.dimension)]
        ...
        self.cell_coord = [[i.__str__() + "," + j.__str__() for i in range(g_var_UR_nma.dimension)] for j in range(g_var_UR_nma.dimension)]


        self.label_poacher_num = Label(self.root, text = "Number of Total \nPoachers:\n" + str(g_var_UR_nma.num_of_adverseries))
        self.label_poacher_num.place(relx=0.78, rely=0.05)

        self.label_arrest = Label(self.root, text = g_var_UR_nma.arrested_poachers)
        self.label_arrest.place(relx=0.78, rely=0.2)
        self.label_fled = Label(self.root, text = g_var_UR_nma.fled_poachers)
        self.label_fled.place(relx=0.78, rely=0.3)
        self.label_sack = Label(self.root, text = g_var_UR_nma.resource_poached)
        self.label_sack.place(relx=0.78, rely=0.4)
        self.label_recovered = Label(self.root, text = g_var_UR_nma.resource_poached)
        self.label_recovered.place(relx=0.78, rely=0.5)
        self.label_travelled = Label(self.root, text = g_var_UR_nma.distance_travelled)
        self.label_travelled.place(relx=0.78, rely=0.6)

        self.label_agent_num = Label(self.root, text = "Number of Agents:\n" + str(g_var_UR_nma.num_of_agents))
        self.label_agent_num.place(relx=0.78, rely=0.7)
        self.label_drone_num = Label(self.root, text = "Number of Drones:\n" + str(g_var_UR_nma.num_of_drones))
        self.label_drone_num.place(relx=0.78, rely=0.8)


        self.refresh()
        self.canvas.create_rectangle(0, 0, g_var_UR_nma.dimension * g_var_UR_nma.block_size, g_var_UR_nma.dimension * g_var_UR_nma.block_size, fill=g_var_UR_nma.bg_color)

        # for ONE TIME labelling *******************************************
        for i in range(g_var_UR_nma.dimension):
            for j in range(g_var_UR_nma.dimension):
                self.coord_label = Label(self.root,text = self.cell_coord[i][j],bg="black",fg="white")
                self.coord_label.place(x=i * g_var_UR_nma.block_size + 2, y=j * g_var_UR_nma.block_size + 18)

        for i in range(g_var_UR_nma.dimension + 1):
            for j in range(g_var_UR_nma.dimension + 1):
                self.canvas.create_rectangle(i * g_var_UR_nma.block_size, j * g_var_UR_nma.block_size, g_var_UR_nma.block_size, g_var_UR_nma.block_size, outline="grey")

        for i in range(g_var_UR_nma.num_of_agents):
        ...
        for i in range(g_var_UR_nma.num_of_drones):
        ...
        for i in range(g_var_UR_nma.num_of_adverseries):
...
print("\n***Simulating Green Security Game (Unbounded guarding + Radio signalling - NMA)***\n")
...
with open('F://MS Thesis Implementation Final GitHub//GSG-Variations//results//UR-nma.csv', 'w') as csvfile:
    title = ['unbounded + radio - nma']
    ...
    for i in range(1,9):
        ...
        for j in range(num_of_trials):
            ...
            adv_in = 10
            guard_in = 8
            drone_in = i #i*g_var_UR.exchange_rate
            ...
            value_list[0][j] += g_var_UR_nma.arrested_poachers
            avg_list[0] += value_list[0][j]
            value_list[1][j] += g_var_UR_nma.fled_poachers
            avg_list[1] += value_list[1][j]
            value_list[2][j] += g_var_UR_nma.resource_poached
            avg_list[2] += value_list[2][j]
            value_list[3][j] += g_var_UR_nma.resource_recovered
            avg_list[3] += value_list[3][j]
            value_list[4][j] += g_var_UR_nma.distance_travelled
        ...
        print("\n******************** The average for trial " + str(g_var_UR_nma.num_of_adverseries) + "," + str(g_var_UR_nma.num_of_agents) + "," + str(g_var_UR_nma.num_of_drones) + " is: ********************\n")
