...
class app(Frame):
    ...
    def __init__(self, _num_adv, _num_agents, _num_drones):
        ...
        self.max_drone_subarea = int(math.ceil(1.00*g_var.num_of_drones/g_var.num_of_agents))
        for i in range(g_var.num_of_agents):
            ...
            for drone_jjj in range(self.max_drone_subarea):
                if self.drone_employed < g_var.num_of_drones:
...
with open('F://MS Thesis Implementation Final GitHub//GSG-Variations//results//bounded+radio.csv', 'w') as csvfile:
    title = ['bounded + radio']
