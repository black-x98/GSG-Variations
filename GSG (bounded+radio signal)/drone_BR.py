...
class drone(entities):
    ...
    def __init__(self,_canvas,_root, _drone_pos, _drone_signal, _adv_pos,_subarea_y,_subarea_x,_sub_y,_sub_x,_subarea_signal):
        ...
        x_cor = self.cur_x_drone * g_var_BR.block_size
        y_cor = self.cur_y_drone * g_var_BR.block_size
        self.drone_pos[self.cur_y_drone][self.cur_x_drone] = 1
        self.canvas.create_oval(x_cor + 22, y_cor + 10, x_cor + 37, y_cor + 25, fill=self.drone_color, outline=g_var_BR.bg_color)
    ...
    def move_drone(self):
        ...
        x_cor = self.cur_x_drone * g_var_BR.block_size
        y_cor = self.cur_y_drone * g_var_BR.block_size
        self.canvas.create_polygon(x_cor + 15, y_cor + 20, x_cor + 15, y_cor + 35, x_cor + 30, y_cor + 35, x_cor + 30, y_cor + 20, fill=g_var_BR.bg_color, outline=g_var_BR.bg_color)
        self.drone_pos[self.cur_y_drone][self.cur_x_drone] = 0
        lower_limit_x = -1
        lower_limit_y = -1
        upper_limit_x = 1
        upper_limit_y = 1
        self.canvas.create_oval(x_cor + 22, y_cor + 10, x_cor + 37, y_cor + 25, fill=g_var_BR.bg_color, outline=g_var_BR.bg_color)
        if self.cur_x_drone<=g_var_BR.dimension*self.subarea_x[self.sub_x][0]:
                lower_limit_x = 0
        elif self.cur_x_drone>=g_var_BR.dimension*self.subarea_x[self.sub_x][1]-1:
            upper_limit_x = 0
        if self.cur_y_drone<=g_var_BR.dimension*self.subarea_y[self.sub_y][0]:
            lower_limit_y = 0
        elif self.cur_y_drone>=g_var_BR.dimension*self.subarea_y[self.sub_y][1]-1:
            upper_limit_y = 0
        ...
        if self.drone_counter < g_var_BR.movement_limit:
            self.root.after(g_var_BR.turn_gap_time, self.move_drone)
