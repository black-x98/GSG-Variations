...
class app(Frame):
    ...
    def __init__(self, _num_adv, _num_agents, _num_drones):
        ...
        frac = 1.00 * g_var.num_of_drones / g_var.num_of_agents
        self.per_subarea_min = int(math.floor(frac))
        self.extra_drone_num = int((frac - math.floor(frac)) * g_var.num_of_agents)
        ...
        for i in range(self.extra_drone_num):
            sub_y = random.randint(0,2)
            sub_x = random.randint(0,2)
            while self.subarea_check_drone[sub_y][sub_x] >= g_var.exchange_rate:
                sub_y = random.randint(0,2)
                sub_x = random.randint(0,2)
            #sub_y = 1
            #sub_x = 1
...
with open('bounded+sound.csv', 'w') as csvfile:
    title = ['bounded + sound']
