...
class drone(entities):
    ...
    def move_drone(self):
        ...
        if self.cur_x_drone<=g_var.dimension*self.sub_mult_x[0]:
                lower_limit_x = 0
        elif self.cur_x_drone>=g_var.dimension*self.sub_mult_x[1]-1:
            upper_limit_x = 0
        if self.cur_y_drone<=g_var.dimension*self.sub_mult_y[0]:
            lower_limit_y = 0
        elif self.cur_y_drone>=g_var.dimension*self.sub_mult_y[1]-1:
            upper_limit_y = 0
