...
class drone(entities):
    ...
    def __init__(self,_canvas,_root, _drone_pos, _drone_signal, _adv_pos):
        ...
        self.cur_x_drone = random.randint(0, g_var_UR.dimension - 2)
        self.cur_y_drone = random.randint(0, g_var_UR.dimension - 2)
    ...
    def move_drone(self):
        ...
        if self.cur_y_drone<1:
            lower_limit_y = 0
        if self.cur_x_drone>g_var_UR.dimension-2:
            upper_limit_x=0
        if self.cur_y_drone>g_var_UR.dimension-2:
            upper_limit_y=0
        ...
        if self.drone_counter < g_var_UR.movement_limit:
            self.root.after(g_var_UR.turn_gap_time, self.move_drone)
