...
from agent_UR import agent
from adversery_UR import adv
from drone_UR import drone
import g_var_UR
...
class app(Frame):
    ...
    def refresh(self):
        ...
        self.label_fled['text'] = "FLED Poachers:\n" + str(g_var_UR.fled_poachers)
        self.label_sack['text'] = "Resource Poached:\n" + str(g_var_UR.resource_poached)
        self.label_recovered['text'] = "Resource Recovered:\n" + str(g_var_UR.resource_recovered)
        self.label_travelled['text'] = "Distance Travelled:\n" + str(g_var_UR.distance_travelled)

        for i in range(g_var_UR.dimension): # cell resource label update
            for j in range(g_var_UR.dimension):
                res_label = Label(self.root,text = self.cell_resources[j][i],bg="black",fg="white")
                res_label.place(x=i * g_var_UR.block_size + 2, y=j * g_var_UR.block_size + 2)

        self.refresh_counter += 1
        if self.refresh_counter <= g_var_UR.movement_limit:
            #print("working in main refresh " + random.randint(1,50).__str__())
            self.root.after(g_var_UR.turn_gap_time, self.refresh)
        elif self.refresh_counter > g_var_UR.movement_limit:
    ...
    def __init__(self, _num_adv, _num_agents, _num_drones):
        ...
        g_var_UR.num_of_adverseries = _num_adv
        g_var_UR.num_of_agents = _num_agents
        g_var_UR.num_of_drones = _num_drones

        # re-initialization of result variables

        g_var_UR.arrested_poachers = 0
        g_var_UR.fled_poachers = 0
        g_var_UR.resource_poached = 0
        g_var_UR.resource_recovered = 0
        g_var_UR.distance_travelled = 0

        self.refresh_counter = 0
        g_var_UR.movement_limit = 40
        ...
        self.agent_pos = [[0 for i in range(g_var_UR.dimension)] for j in range(g_var_UR.dimension)]
        ...
        self.label_poacher_num = Label(self.root, text = "Number of Total \nPoachers:\n" + str(g_var_UR.num_of_adverseries))
        self.label_poacher_num.place(relx=0.78, rely=0.05)

        self.label_arrest = Label(self.root, text = g_var_UR.arrested_poachers)
        self.label_arrest.place(relx=0.78, rely=0.2)
        self.label_fled = Label(self.root, text = g_var_UR.fled_poachers)
        self.label_fled.place(relx=0.78, rely=0.3)
        self.label_sack = Label(self.root, text = g_var_UR.resource_poached)
        self.label_sack.place(relx=0.78, rely=0.4)
        self.label_recovered = Label(self.root, text = g_var_UR.resource_poached)
        self.label_recovered.place(relx=0.78, rely=0.5)
        self.label_travelled = Label(self.root, text = g_var_UR.distance_travelled)
        self.label_travelled.place(relx=0.78, rely=0.6)

        self.label_agent_num = Label(self.root, text = "Number of Agents:\n" + str(g_var_UR.num_of_agents))
        self.label_agent_num.place(relx=0.78, rely=0.7)
        self.label_drone_num = Label(self.root, text = "Number of Drones:\n" + str(g_var_UR.num_of_drones))
        self.label_drone_num.place(relx=0.78, rely=0.8)


        self.refresh()
        self.canvas.create_rectangle(0, 0, g_var_UR.dimension * g_var_UR.block_size, g_var_UR.dimension * g_var_UR.block_size, fill=g_var_UR.bg_color)
        ...
        for i in range(g_var_UR.dimension):
            for j in range(g_var_UR.dimension):
                ...
                self.coord_label.place(x=i * g_var_UR.block_size + 2, y=j * g_var_UR.block_size + 18)

        for i in range(g_var_UR.dimension + 1):
            for j in range(g_var_UR.dimension + 1):
        ...
        for i in range(g_var_UR.num_of_drones):
...
with open('F://MS Thesis Implementation Final GitHub//GSG-Variations//results//UR.csv', 'w') as csvfile:
    ...
    for i in range(1,9):
        ...
        for j in range(num_of_trials):
            ...
            value_list[1][j] += g_var_UR.fled_poachers
            avg_list[1] += value_list[1][j]
            value_list[2][j] += g_var_UR.resource_poached
            avg_list[2] += value_list[2][j]
            value_list[3][j] += g_var_UR.resource_recovered
            avg_list[3] += value_list[3][j]
            value_list[4][j] += g_var_UR.distance_travelled
        ...
        print("\n******************** The average for trial " + str(g_var_UR.num_of_adverseries) + "," + str(g_var_UR.num_of_agents) + "," + str(g_var_UR.num_of_drones) + " is: ********************\n")
