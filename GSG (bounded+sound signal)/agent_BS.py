...
class agent(entities):
    ...
    def __init__(self,_canvas,_root,_agent_pos,_cell_resources,_target_pos,_round_marking,_drone_signal,_sub_mult_y,_sub_mult_x, _adv_pos):
        ...
        self.cur_y_agent = random.randint(round(g_var_BS.dimension * self.sub_mult_y[0]), round(g_var_BS.dimension * self.sub_mult_y[1]) - 1)
        self.cur_x_agent = random.randint(round(g_var_BS.dimension * self.sub_mult_x[0]), round(g_var_BS.dimension * self.sub_mult_x[1]) - 1)
    ...
    def move_agent(self):
        self.move_skip = (self.move_skip + 1) % g_var_BS.move_skip_toggle
        ...
        for i in range(3):
            for j in range(3):
                ...
                if temp_y>=g_var_BS.dimension*self.sub_mult_y[0] and temp_y<=g_var_BS.dimension*self.sub_mult_y[1]-1 and temp_x>=g_var_BS.dimension*self.sub_mult_x[0] and temp_x<=g_var_BS.dimension*self.sub_mult_x[1]-1:
        ...
        if self.active_target==True:
            ...
            if self.cur_x_agent<=g_var_BS.dimension*self.sub_mult_x[0]:
                lower_limit_x = 0
            elif self.cur_x_agent>=g_var_BS.dimension*self.sub_mult_x[1]-1:
                upper_limit_x = 0
            if self.cur_y_agent<=g_var_BS.dimension*self.sub_mult_y[0]:
                lower_limit_y = 0
            elif self.cur_y_agent>=g_var_BS.dimension*self.sub_mult_y[1]-1:
                upper_limit_y = 0
