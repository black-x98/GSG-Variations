...
class adv(entities):
    ...
    def __init__(self,_canvas,_root, _agent_pos, _drone_pos, _cell_resources, _target_pos, _adv_pos):
        ...
        set = {0, g_var_BS.dimension - 1}
        if random.randint(0,1)==0:
            self.cur_x_adv = random.sample(set,1)[0]
            self.cur_y_adv = random.randint(0, g_var_BS.dimension - 1)
        else:
            self.cur_y_adv = random.sample(set,1)[0]
            self.cur_x_adv = random.randint(0, g_var_BS.dimension - 1)

        x_cor = self.cur_x_adv * g_var_BS.block_size
        y_cor = self.cur_y_adv * g_var_BS.block_size
    ...
    def move_adv(self):
        x_cor = self.cur_x_adv * g_var_BS.block_size
        y_cor = self.cur_y_adv * g_var_BS.block_size
        self.canvas.create_polygon(x_cor + 15, y_cor + 38, x_cor + 35, y_cor + 38, x_cor + 25, y_cor + 19, fill=g_var_BS.bg_color, outline=g_var_BS.bg_color)
        ...
        move_x, move_y = 0,0
        if x_offset>0 and self.cur_x_adv<g_var_BS.dimension-1: move_x = +1
        elif x_offset<0 and self.cur_x_adv>0 : move_x=-1
        if y_offset>0 and self.cur_y_adv<g_var_BS.dimension-1: move_y = +1
        elif y_offset<0 and self.cur_y_adv>0 : move_y=-1

        self.cur_x_adv += move_x
        self.cur_y_adv += move_y

        if self.cur_x_adv>g_var_BS.dimension-1 or self.cur_y_adv>g_var_BS.dimension-1: # invalid move check!!!
        ...
        x_cor = self.cur_x_adv * g_var_BS.block_size
        y_cor = self.cur_y_adv * g_var_BS.block_size
        self.canvas.create_polygon(x_cor + 15, y_cor + 38, x_cor + 35, y_cor + 38, x_cor + 25, y_cor + 19, fill=self.adv_color, outline=g_var_BS.bg_color)
        self.adv_pos[self.cur_y_adv][self.cur_x_adv] = 1
    ...
    def fix_escape_point(self):
        ...
        down_end = g_var_BS.dimension - 1 - self.cur_y_adv
        right_end = g_var_BS.dimension - 1 - self.cur_x_adv
        ...
        if up_end <= down_end:
            ...
            self.escape_y = g_var_BS.dimension - 1
            self.small_y = down_end
        if left_end <= right_end :
            self.escape_x = 0
            self.small_x = left_end
        else:
            self.escape_x = g_var_BS.dimension - 1
    ...
    def escape(self):
        x_cor = self.cur_x_adv * g_var_BS.block_size
        y_cor = self.cur_y_adv * g_var_BS.block_size
        self.canvas.create_polygon(x_cor + 15, y_cor + 38, x_cor + 35, y_cor + 38, x_cor + 25, y_cor + 19, fill=g_var_BS.bg_color, outline="yellow")
        self.adv_pos[self.cur_y_adv][self.cur_x_adv] = 0
        self.flag = 5
    ...
    def operate_adv(self):
        ...
        if self.flag == 5:
            ...
            g_var_BS.arrested_poachers += 1
            g_var_BS.resource_poached -= self.sack
            g_var_BS.resource_recovered += self.sack
            ...
            y_cor = self.cur_y_adv * g_var_BS.block_size
            self.canvas.create_polygon(x_cor+15,y_cor+38,x_cor+35,y_cor+38,x_cor+25,y_cor+19,fill=self.adv_color)
            if self.adv_move_counter < g_var_BS.movement_limit:
                self.root.after(g_var_BS.turn_gap_time, self.operate_adv)
