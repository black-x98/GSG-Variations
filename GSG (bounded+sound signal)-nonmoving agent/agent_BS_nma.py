...
class agent(entities):
    ...
    def __init__(self,_canvas,_root,_agent_pos,_cell_resources,_target_pos,_round_marking,_drone_signal,_sub_mult_y,_sub_mult_x, _adv_pos):
        ...
        self.cur_y_agent = random.randint(round(g_var_BS_nma.dimension * self.sub_mult_y[0]), round(g_var_BS_nma.dimension * self.sub_mult_y[1]) - 1)
        self.cur_x_agent = random.randint(round(g_var_BS_nma.dimension * self.sub_mult_x[0]), round(g_var_BS_nma.dimension * self.sub_mult_x[1]) - 1)


        self.my_target = _target_pos[random.randint(0,len(_target_pos)-1)]

        x_cor = self.cur_x_agent * g_var_BS_nma.block_size
        y_cor = self.cur_y_agent * g_var_BS_nma.block_size
    ...
    def move_agent(self):
        self.move_skip = (self.move_skip + 1) % g_var_BS_nma.move_skip_toggle
        ...
        for i in range(3):
            for j in range(3):
                ...
                if temp_y>=g_var_BS_nma.dimension*self.sub_mult_y[0] and temp_y<=g_var_BS_nma.dimension*self.sub_mult_y[1]-1 and temp_x>=g_var_BS_nma.dimension*self.sub_mult_x[0] and temp_x<=g_var_BS_nma.dimension*self.sub_mult_x[1]-1:
        ...
        if self.active_target==True:
            ...
            x_cor = self.cur_x_agent * g_var_BS_nma.block_size
            y_cor = self.cur_y_agent * g_var_BS_nma.block_size
            self.canvas.create_polygon(x_cor + 15, y_cor + 20, x_cor + 15, y_cor + 35, x_cor + 30, y_cor + 35, x_cor + 30, y_cor + 20, fill=g_var_BS_nma.bg_color, outline=g_var_BS_nma.bg_color)
            ...
            move_x, move_y = 0,0
            if x_offset>0 and self.cur_x_agent<g_var_BS_nma.dimension-1: move_x = +1
            elif x_offset<0 and self.cur_x_agent>0 : move_x=-1
            if y_offset>0 and self.cur_y_agent<g_var_BS_nma.dimension-1: move_y = +1
            ...
            g_var_BS_nma.distance_travelled += 1
            ...
            if self.cur_x_agent<=g_var_BS_nma.dimension*self.sub_mult_x[0]:
                lower_limit_x = 0
            elif self.cur_x_agent>=g_var_BS_nma.dimension*self.sub_mult_x[1]-1:
                upper_limit_x = 0
            if self.cur_y_agent<=g_var_BS_nma.dimension*self.sub_mult_y[0]:
                lower_limit_y = 0
            elif self.cur_y_agent>=g_var_BS_nma.dimension*self.sub_mult_y[1]-1:
                upper_limit_y = 0
        ...
        if self.agent_counter < g_var_BS_nma.movement_limit:
            self.root.after(g_var_BS_nma.turn_gap_time, self.move_agent)
